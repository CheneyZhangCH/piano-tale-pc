<template>
    <section class="section-item">
        <autocomplete
            ...
            :popper-class="subTitle ? ('aika-fuzzy-multi-select ' + item.className) : 'aika-fuzzy-multi-select'"
        ...
        >
            <template v-if="item.optionCustomTipClick" #default="scope">
                <div v-if="scope.item.type === 'customTipClick'" class="custom-tips">
                    <span>{{ scope.item.text }}</span>
                    <span class="custom-tips-btn" @click="scope.item.func">{{ scope.item.btn }}</span>
                </div>
                <div v-else>
                    <div v-if="scope.item.itemIndex === 0 && item.optionCustomTipVisible" class="custom-tips" v-html="item.optionCustomTip" />
